...
import '-core/contracts/interfaces/Factory.sol';
import '-core/contracts/interfaces/Pair.sol';
import '-core/contracts/interfaces/IERC20.sol';
...
contract Router is Router {
  ...
  function removeLiquidityWithPermit(
    ...
    Pair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
  ...
  function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to, address referrer) internal {
    for (uint i; i < path.length - 1; i++) {
      ...
      Pair pair = Pair(UniswapV2Library.pairFor(factory, input, output));
      uint amountOutput;
